...
export class AuthService {
  ...
  async resetPassword(dto: ResetPasswordDto) {
    ...
    if (!validReset) {
      const expiredReset = await this.prisma.passwordReset.findFirst({
        where: { tokenHash, usedAt: null, expiresAt: { lte: new Date() } },
      });
      if (expiredReset) throw new TokenExpiredException();
