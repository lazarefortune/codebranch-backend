...
function getPageId(body: unknown): string {
  const typedBody = body as CreatePageResponseBody;
  if (typeof typedBody.page?.id !== 'string') {
    throw new Error('Missing page.id in create page response');
  }
  return typedBody.page.id;
}
...
describe('PagesController (e2e)', () => {
  ...
  beforeAll(async () => {
    ...
    accessToken = getAccessToken(loginResponse.body);
  }, 30000);

  afterAll(async () => {
    try {
      await prisma.block.deleteMany({
        where: { page: { userId: testUserId } },
      });
  ...
  describe('POST /api/v1/pages', () => {
    it('should create a new page', async () => {
      ...
      createdPageId = getPageId(response.body);
    });
  ...
  describe('GET /api/v1/pages/:pageId', () => {
    ...
    it('should return 404 for non-existent page', async () => {
      await request(app.getHttpServer())
        .get('/api/v1/pages/non-existent-id')
        .set('Authorization', 'Bearer ' + accessToken)
        .expect(404);
    });
  });
  ...
  describe('DELETE/api/v1/pages/:pageId', () => {
    it('should delete the page', async () => {
      await request(app.getHttpServer())
        .delete('/api/v1/pages/' + createdPageId)
        .set('Authorization', 'Bearer ' + accessToken)
        .expect(204);
    });
